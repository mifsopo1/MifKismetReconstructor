...
// Pin category FNames (mirror UEdGraphSchema_K2 without dragging in BlueprintGraph).
...
const FName PC_Real(TEXT("real"));     // UE5: float/double pins are PC_Real + subcategory
const FName PC_Float(TEXT("float"));   // now a PinSubCategory in UE5 (kept as name too)
const FName PC_Double(TEXT("double")); // PinSubCategory for double-precision
...
// Both fatals here were reachable from arbitrary cooked bytecode — the sweep deserializes pin types that name
// cooked-only / renamed / other-pak / editor-unloadable assets, and ANY such path took the editor down. All three call
// sites assign the result straight into a pin-type field (PinSubCategoryObject / MemberParent /
// TerminalSubCategoryObject), where null IS the already-handled "unresolved" state (bad_type/None downstream).
UObject* DeserializeObjectRef(const FString& ObjectPath, UClass* SelfScope) {
	if (ObjectPath == TEXT("<SELF>")) {
		if (!SelfScope) {
			UE_LOG(LogTemp, Warning, TEXT("[MifKR] <SELF> object reference with no self scope (deferred: type left unresolved)"));
		}
		return SelfScope;
	}
	UObject* LoadedObject = LoadObject<UObject>(NULL, *ObjectPath);
	if (!LoadedObject) {
		UE_LOG(LogTemp, Warning, TEXT("[MifKR] Failed to resolve object reference (deferred: type left unresolved): %s"), *ObjectPath);
	}
	return LoadedObject;
}
...
TSharedRef<FJsonObject> FPropertyTypeHelper::SerializeGraphPinType(const FEdGraphPinType& GraphPinType, UClass* SelfScope) {
	...
	// FIX (was copy-paste of PinCategory in the original toolkit):
	TypeEntry->SetStringField(TEXT("PinSubCategory"), GraphPinType.PinSubCategory.ToString());
...
bool GetPropertyCategoryInfo(const FProperty* TestProperty, FName& OutCategory, FName& OutSubCategory, UObject*& OutSubCategoryObject, bool& bOutIsWeakPointer) {
	if (const FInterfaceProperty* InterfaceProperty = CastField<const FInterfaceProperty>(TestProperty)) {
		...
		// UE5: float pins are PC_Real with a "float" subcategory (was bare PC_Float in UE4).
		OutCategory = PC_Real;
		OutSubCategory = PC_Float;
	} else if (TestProperty->IsA<FDoubleProperty>()) {
		// UE5 LWC: double pins are PC_Real with a "double" subcategory. (Missing in the UE4 toolkit.)
		OutCategory = PC_Real;
